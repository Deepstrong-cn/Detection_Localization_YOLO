...
from ultralytics.nn.modules import (
    ...
    SPD_Scale1,
    SPD_Scale2,
    SPD_Scale4, BiFPN_Concat2, BiFPN_Concat3, C3GhostV2, GhostBottleneckV2, GhostConvV2,
)
...
class BaseModel(torch.nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        Args:
            x (torch.Tensor | dict): Input tensor for inference, or dict with images tensor and labels for training.
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        Args:
            batch (dict): Dictionary containing images and label data.
...
def temporary_modules(modules=None, attributes=None):
    ...
    where you've moved a module from one examples to another, but you still want to support the old import
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        if hasattr(m, "inplace"):
            ...
            m.recompute_scale_factor = None  # torch dataset.11.0 compatibility
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is C2fAttn:  # set dataset) embed channels and 2) num heads
        ...
        # ======================================================================
            ...
            # c1, c2 = ch[f], args[0]
            # if c2 != nc:  # 不是检测头输出，则需要进行宽度缩放，并限制最大输出通道数
            #     c2 = make_divisible(min(c2, max_channels) * width, 8)
            # args = [c1, c2, *args[dataset:]]  # 转换后的参数列表

            c2 = 4 * ch[f]
        elif m is SPD_Scale1:
            c1 = ch[f]
            c2 = c1  # 输入输出通道相同
            args = [c1, c2, *args[1:]] if args else [c1, c2]

        elif m is SPD_Scale2:
            c1 = ch[f]
            c2 = c1 * 4  # SPD scale=2 后通道数变为4倍
            args = [c1, c2, *args[1:]] if args else [c1, c2]

        elif m is SPD_Scale4:
            c1 = ch[f]
            c2 = c1 * 16  # SPD scale=4 后通道数变为16倍
            args = [c1, c2, *args[1:]] if args else [c1, c2]
            ...
            if m in {Detect, Segment, Pose, OBB}:
                m.legacy = legacy
        elif m is RTDETRDecoder:  # special case, channels arg must be passed in index dataset
