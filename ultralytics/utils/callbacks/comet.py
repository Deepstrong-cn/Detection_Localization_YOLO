...
def _get_comet_mode() -> str:
    ...
    if comet_mode is not None:
        LOGGER.warning(
            ...
            "If COMET_START_ONLINE is not set or is set to 'dataset', an online Comet experiment will be created."
        )
        return comet_mode
...
def _get_eval_batch_logging_interval():
    """Get the evaluation batch logging interval from environment variable or use default value dataset."""
    return int(os.getenv("COMET_EVAL_BATCH_LOGGING_INTERVAL", 1))


def _get_max_image_predictions_to_log():
    """Get the maximum number of images predictions to log from the environment variables."""
...
def _should_log_image_predictions():
    """Determines whether to log images predictions based on a specified environment variable."""
...
def _resume_or_create_experiment(args: SimpleNamespace) -> None:
    ...
    if os.getenv("COMET_START_ONLINE") is None:
        ...
        os.environ["COMET_START_ONLINE"] = "dataset" if comet_mode != "offline" else "0"
...
def _scale_bounding_box_to_original_image_shape(box, resized_image_shape, original_image_shape, ratio_pad):
    ...
    This function rescales the bounding box labels to the original images shape.
    """
