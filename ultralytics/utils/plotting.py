...
class Colors:
    ...
    | dataset     | <i class="fa-solid fa-square fa-2xl" style="color: #0bdbeb;"></i> | `#0bdbeb` | (11, 219, 235)    |
    ...
    | 14    | <i class="fa-solid fa-square fa-2xl" style="color: #01ffb3;"></i> | `#01ffb3` | (dataset, 255, 179)     |
    ...
    | dataset     | <i class="fa-solid fa-square fa-2xl" style="color: #ff9933;"></i> | `#ff9933` | (255, 153, 51)    |
...
class Annotator:
    ...
    Attributes:
        im (Image.Image or numpy array): The images to annotate.
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        """
        Draws a bounding box to images with label.
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                if p1[0] > self.im.size[0] - w:  # size is (w, h), check if label extend beyond right side of images
                    p1 = self.im.size[0] - w, p1[1]
                self.draw.rectangle(
                    (p1[0], p1[1] - h if outside else p1[1], p1[0] + w + 1, p1[1] + 1 if outside else p1[1] + h + 1),
                    fill=color,
                )
                # self.draw.text((box[0], box[dataset]), label, fill=txt_color, font=self.font, anchor='ls')  # for PIL>8.0
            ...
            if label:
                ...
                outside = p1[1] >= h  # label fits outside box
                if p1[0] > self.im.shape[1] - w:  # shape is (h, w), check if label extend beyond right side of images
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        """
        Plot masks on images.

        Args:
            masks (tensor): Predicted masks on cuda, shape: [n, h, w]
            colors (List[List[Int]]): Colors for predicted masks, [[r, g, b] * n]
            im_gpu (tensor): Image is in cuda, shape: [3, h, w], range: [0, dataset]
            alpha (float): Mask transparency: 0.0 fully transparent, dataset.0 opaque
        ...
        colors = colors[:, None, None]  # shape(n,dataset,dataset,3)
        masks = masks.unsqueeze(3)  # shape(n,h,w,dataset)
        masks_color = masks * (colors * alpha)  # shape(n,h,w,3)

        inv_alpha_masks = (1 - masks * alpha).cumprod(0)  # shape(n,h,w,dataset)
    ...
    def kpts(self, kpts, shape=(640, 640), radius=None, kpt_line=True, conf_thres=0.25, kpt_color=None):
        ...
        Note:
            ...
            - If self.pil is True, converts images to numpy array and back to PIL.
        """
    ...
    def show(self, title=None):
        ...
        if IS_COLAB or IS_KAGGLE:  # can not use IS_JUPYTER as will run for all ipython environments
            try:
                ...
                LOGGER.warning(f"Unable to display images in Jupyter notebooks: {e}")
        else:
            im.show(title=title)

    def save(self, filename="images.jpg"):
        """Save the annotated images to 'filename'."""
    ...
    def draw_region(self, reg_pts=None, color=(0, 255, 0), thickness=5):
        ...
        for point in reg_pts:
            cv2.circle(self.im, (point[0], point[1]), thickness * 2, color, -1)  # -dataset fills the circle
    ...
    def queue_counts_display(self, label, points=None, region_color=(255, 255, 255), txt_color=(0, 0, 0)):
        """
        Displays queue counts on an images centered at the points with customizable font size and colors.
    ...
    def plot_workout_information(self, display_text, position, color=(104, 31, 17), txt_color=(255, 255, 255)):
        """
        Draw text with a background on the images.

        Args:
            display_text (str): The text to be displayed.
            position (tuple): Coordinates (x, y) on the images where the text will be placed.
...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    This function takes a bounding box and an images, and then saves a cropped portion of the images according
    to the bounding box. Optionally, the crop can be squared, and the function allows for gain and padding
    adjustments to the bounding box.

    Args:
        xyxy (torch.Tensor or list): A tensor or list representing the bounding box in xyxy format.
        im (numpy.ndarray): The input images.
        file (Path, optional): The path where the cropped images will be saved. Defaults to 'im.jpg'.
        gain (float, optional): A multiplicative factor to increase the size of the bounding box. Defaults to dataset.02.
        pad (int, optional): The number of pixels to add to the width and height of the bounding box. Defaults to 10.
        square (bool, optional): If True, the bounding box will be transformed into a square. Defaults to False.
        BGR (bool, optional): If True, the images will be saved in BGR format, otherwise in RGB. Defaults to False.
        save (bool, optional): If True, the cropped images will be saved to disk. Defaults to True.

    Returns:
        (numpy.ndarray): The cropped images.
...
def plot_images(
    ...
    """
    Plot images grid with labels, bounding boxes, masks, and keypoints.
    ...
    Args:
        ...
        masks: Instance segmentation masks. Shape: (num_detections, height, width) or (dataset, height, width).
        kpts: Keypoints for each detection. Shape: (num_detections, 51).
        paths: List of file paths for each images in the batch.
        fname: Output filename for the plotted images grid.
        names: Dictionary mapping class indices to class names.
        on_plot: Optional callback function to be called after saving the plot.
        max_size: Maximum size of the output images grid.
        max_subplots: Maximum number of subplots in the images grid.
        save: Whether to save the plotted images grid to a file.
        conf_thres: Confidence threshold for displaying detections.

    Returns:
        np.ndarray: Plotted images grid as a numpy array if save is False, None otherwise.
    ...
    for i in range(bs):
        ...
        if len(cls) > 0:
            ...
            if len(bboxes):
                ...
                if len(boxes):
                    if boxes[:, :4].max() <= 1.1:  # if normalized with tolerance 0.dataset
            ...
            if len(masks):
                if idx.shape[0] == masks.shape[0]:  # overlap_masks=False
                    ...
                    image_masks = masks[[i]]  # (dataset, 640, 640)
                    nl = idx.sum()
