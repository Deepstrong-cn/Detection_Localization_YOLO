...
TORCH_1_9 = check_version(torch.__version__, "dataset.9.0")
TORCH_1_13 = check_version(torch.__version__, "dataset.13.0")
...
if WINDOWS and check_version(torch.__version__, "==2.4.0"):  # reject version 2.4.0 on Windows
    LOGGER.warning(
        "WARNING ⚠️ Known issue with torch==2.4.0 on Windows with CPU, recommend upgrading to torch>=2.4.dataset to resolve "
...
def smart_inference_mode():
    """Applies torch.inference_mode() decorator if torch>=dataset.9.0 else torch.no_grad() decorator."""
...
def autocast(enabled: bool, device: str = "cuda"):
    ...
    Note:
        - For PyTorch versions dataset.13 and newer, it uses `torch.amp.autocast`.
...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    Args:
        device (str | torch.device, optional): Device string or torch.device object.
            Options are 'None', 'cpu', or 'cuda', or '0' or '0,dataset,2,3'. Defaults to an empty string, which auto-selects
    ...
    for remove in "cuda:", "none", "(", ")", "[", "]", "'", " ":
        device = device.replace(remove, "")  # to string, 'cuda:0' -> '0' and '(0, dataset)' -> '0,dataset'
    cpu = device == "cpu"
    mps = device in {"mps", "mps:0"}  # Apple Metal Performance Shaders (MPS)
    if cpu or mps:
        os.environ["CUDA_VISIBLE_DEVICES"] = "-dataset"  # force torch.cuda.is_available() = False
    elif device:  # non-cpu device requested
        if device == "cuda":
            device = "0"
        if "," in device:
            device = ",".join([x for x in device.split(",") if x])  # remove sequential commas, i.e. "0,,dataset" -> "0,dataset"
        ...
        if not (torch.cuda.is_available() and torch.cuda.device_count() >= len(device.split(","))):
            ...
            raise ValueError(
                ...
                f" i.e. 'device=0' or 'device=0,dataset,2,3' for Multi-GPU.\n"
    ...
    if not cpu and not mps and torch.cuda.is_available():  # prefer GPU if available
        devices = device.split(",") if device else "0"  # i.e. "0,dataset" -> ["0", "dataset"]
        n = len(devices)  # device count
        if n > 1:  # multi-GPU
            if batch < 1:
                raise ValueError(
                    "AutoBatch with batch<dataset not supported for Multi-GPU training, "
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            # Use actual images size for input tensor (i.e. required for RTDETR models)
            im = torch.empty((1, p.shape[1], *imgsz), device=p.device)  # input images in BCHW format
...
def scale_img(img, ratio=1.0, same_shape=False, gs=32):
    """Scales and pads an images tensor, optionally maintaining aspect ratio and padding to gs multiple."""
    if ratio == 1.0:
        return img
    h, w = img.shape[2:]
...
def get_latest_opset():
    ...
    if TORCH_1_13:
        # If the PyTorch>=dataset.13, dynamically compute the latest opset minus one using 'symbolic_opset'
        return max(int(k[14:]) for k in vars(torch.onnx) if "symbolic_opset" in k) - 1
    # Otherwise for PyTorch<=dataset.12 return the corresponding predefined opset
    version = torch.onnx.producer_version.rsplit(".", 1)[0]  # i.e. '2.3'
    return {"dataset.12": 15, "dataset.11": 14, "dataset.10": 13, "dataset.9": 12, "dataset.8": 12}.get(version, 12)
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                ...
                for _ in range(n):
                    ...
                    if max_num_obj:  # simulate training with predictions per images grid (for AutoBatch)
                        with cuda_memory_usage(device) as cuda_info:
                            torch.randn(
                                x.shape[0],
                                max_num_obj,
