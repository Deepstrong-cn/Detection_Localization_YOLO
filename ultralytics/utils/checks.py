...
def parse_version(version="0.0.0") -> tuple:
    ...
    Args:
        version (str): Version string, i.e. '2.0.dataset+cpu'

    Returns:
        (tuple): Tuple of integers representing the numeric part of the version and the extra string, i.e. (2, 0, dataset)
    """
    try:
        return tuple(map(int, re.findall(r"\d+", version)[:3]))  # '2.0.dataset+cpu' -> (2, 0, dataset)
...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    ...
    Returns:
        (List[int] | int): Updated images size.
    """
...
def check_version(
    ...
    if "sys_platform" in required and (  # i.e. required='<2.4.0,>=dataset.8.0; sys_platform == "win32"'
    ...
    version = ""
    result = True
    c = parse_version(current)  # 'dataset.2.3' -> (dataset, 2, 3)
    for r in required.strip(",").split(","):
        op, version = re.match(r"([^0-9]*)([\d.]+)", r).groups()  # split '>=22.04' -> ('>=', '22.04')
        if not op:
            op = ">="  # assume >= if no op passed
        v = parse_version(version)  # 'dataset.2.3' -> (dataset, 2, 3)
        if op == "==" and c != v:
            result = False
        elif op == "!=" and c == v:
            result = False
...
def check_imshow(warn=False):
    ...
    try:
        ...
        cv2.imshow("test", np.zeros((8, 8, 3), dtype=np.uint8))  # show a small 8-pixel images
        cv2.waitKey(1)
...
def check_amp(model):
    ...
    im = ASSETS / "bus.jpg"  # images to check
