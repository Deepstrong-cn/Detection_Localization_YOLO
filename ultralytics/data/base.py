...
class BaseDataset(Dataset):
    ...
    Args:
        ...
        fraction (float): Fraction of dataset to utilize. Default is dataset.0 (use all data).
    ...
    def get_img_files(self, img_path):
        ...
        try:
            ...
            # self.img_files = sorted([x for x in f if x.suffix[dataset:].lower() in IMG_FORMATS])  # pathlib
    ...
    def load_image(self, i, rect_mode=True):
        """Loads dataset images from dataset index 'i', returns (im, resized hw)."""
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    ...
                    LOGGER.warning(f"{self.prefix}WARNING ⚠️ Removing corrupt *.npy images file {fn} due to: {e}")
            ...
            elif not (h0 == w0 == self.imgsz):  # resize by stretching images to square imgsz
    ...
    def check_cache_disk(self, safety_margin=0.5):
        """Check images caching requirements vs available disk space."""
        import shutil
