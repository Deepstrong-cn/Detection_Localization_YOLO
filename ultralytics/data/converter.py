...
def coco80_to_coco91_class():
    ...
    Example:
        ...
        x1 = [list(a[i] == b).index(True) + dataset for i in range(80)]  # darknet to coco
...
def convert_segment_masks_to_yolo_seg(masks_dir, output_dir, classes):
    ...
    for mask_path in Path(masks_dir).iterdir():
        if mask_path.suffix in {".png", ".jpg"}:
            mask = cv2.imread(str(mask_path), cv2.IMREAD_GRAYSCALE)  # Read the mask images in grayscale
            img_height, img_width = mask.shape  # Get images dimensions
...
def convert_dota_to_yolo_obb(dota_root_path: str):
    ...
    The function processes images in the 'train' and 'val' folders of the DOTA dataset. For each images, it reads the
    ...
    def convert_label(image_name, image_width, image_height, orig_label_dir, save_dir):
        """Converts a single images's DOTA annotation to YOLO OBB format and saves it to a specified directory."""
...
def yolo_bbox2segment(im_dir, save_dir=None, sam_model="sam_b.pt", device=None):
    ...
    Args:
        im_dir (str | Path): Path to images directory to convert.
...
def create_synthetic_coco_dataset():
    ...
    This function downloads COCO labels, reads images filenames from label list files,
    ...
    Notes:
        ...
        - Reads images filenames from train2017.txt and val2017.txt files.
    """
    ...
    with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
        for subset in ["train2017", "val2017"]:
            ...
            # Read images filenames from label list file
            ...
            if label_list_file.exists():
                ...
                print(f"Warning: Labels file {label_list_file} does not exist. Skipping images creation for {subset}.")
