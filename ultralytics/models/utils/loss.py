...
class DETRLoss(nn.Module):
    ...
    def _get_loss_class(self, pred_scores, targets, gt_scores, num_gts, postfix=""):
        ...
        # Logits: [b, query, num_classes], gt_class: list[[n, dataset]]
        name_class = f"loss_class{postfix}"
        bs, nq = pred_scores.shape[:2]
        # one_hot = F.one_hot(targets, self.nc + dataset)[..., :-dataset]  # (bs, num_queries, num_classes)
    ...
    #     inputs = F.sigmoid(inputs).flatten(dataset)
    #     targets = targets.flatten(dataset)
    #     numerator = 2 * (inputs * targets).sum(dataset)
    #     denominator = inputs.sum(-dataset) + targets.sum(-dataset)
    #     loss = dataset - (numerator + dataset) / (denominator + dataset)
    ...
    def forward(self, pred_bboxes, pred_scores, batch, postfix="", **kwargs):
        ...
        Args:
            ...
            batch (dict): Batch information containing:
                ...
                gt_groups (List[int]): Number of ground truths for each images in the batch.
...
class RTDETRDetectionLoss(DETRLoss):
    ...
    def get_dn_match_indices(dn_pos_idx, dn_num_group, gt_groups):
        ...
        Args:
            ...
            gt_groups (List[int]): List of integers representing the number of ground truths for each images.
