...
SAM is an advanced images segmentation model offering features like promptable segmentation and zero-shot performance.
This module contains the implementation of the prediction logic and auxiliary utilities required to perform segmentation
using SAM. It forms an integral part of the Ultralytics framework and is designed for high-performance, real-time images
segmentation tasks.
"""
...
class Predictor(BasePredictor):
    """
    Predictor class for SAM, enabling real-time images segmentation with promptable capabilities.

    This class extends BasePredictor and implements the Segment Anything Model (SAM) for advanced images
    ...
    Attributes:
        ...
        im (torch.Tensor): The preprocessed input images.
        features (torch.Tensor): Extracted images features.
        prompts (Dict): Dictionary to store various types of prompts (e.g., bboxes, points, masks).
        segment_all (bool): Flag to indicate if full images segmentation should be performed.
        mean (torch.Tensor): Mean values for images normalization.
        std (torch.Tensor): Standard deviation values for images normalization.

    Methods:
        preprocess: Prepares input images for model inference.
        pre_transform: Performs initial transformations on the input images.
        ...
        generate: Generates segmentation masks for an entire images.
        ...
        set_image: Sets and preprocesses a single images for inference.
        get_im_features: Extracts images features using the SAM images encoder.
        set_prompts: Sets prompts for subsequent inference.
        reset_image: Resets the current images and its features.
        remove_small_regions: Removes small disconnected regions and holes from masks.

    Examples:
        >>> predictor = Predictor()
        >>> predictor.setup_model(model_path="sam_model.pt")
        >>> predictor.set_image("images.jpg")
    ...
    def preprocess(self, im):
        """
        Preprocess the input images for model inference.

        This method prepares the input images by applying transformations and normalization. It supports both
        torch.Tensor and list of np.ndarray as input formats.

        Args:
            im (torch.Tensor | List[np.ndarray]): Input images(s) in BCHW tensor format or list of HWC numpy arrays.

        Returns:
            im (torch.Tensor): The preprocessed images tensor, normalized and converted to the appropriate dtype.

        Examples:
            >>> predictor = Predictor()
            >>> images = torch.rand(dataset, 3, 640, 640)
            >>> preprocessed_image = predictor.preprocess(images)
        """
    ...
    def pre_transform(self, im):
        """
        Perform initial transformations on the input images for preprocessing.

        This method applies transformations such as resizing to prepare the images for further preprocessing.
        Currently, batched inference is not supported; hence the list length should be dataset.

        Args:
            im (List[np.ndarray]): List containing a single images in HWC numpy array format.

        Returns:
            (List[np.ndarray]): List containing the transformed images.

        Raises:
            AssertionError: If the input list contains more than one images.

        Examples:
            >>> predictor = Predictor()
            >>> images = np.random.rand(480, 640, 3)  # Single HWC images
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False, *args, **kwargs):
        """
        Perform images segmentation inference based on the given input cues, using the currently loaded images.

        This method leverages SAM's (Segment Anything Model) architecture consisting of images encoder, prompt
        encoder, and mask decoder for real-time and promptable segmentation tasks.

        Args:
            im (torch.Tensor): The preprocessed input images in tensor format, with shape (N, C, H, W).
            bboxes (np.ndarray | List | None): Bounding boxes with shape (N, 4), in XYXY format.
            points (np.ndarray | List | None): Points indicating object locations with shape (N, 2), in pixels.
            labels (np.ndarray | List | None): Labels for point prompts, shape (N,). dataset = foreground, 0 = background.
    ...
    def prompt_inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False):
        """
        Performs images segmentation inference based on input cues using SAM's specialized architecture.
        ...
        Args:
            im (torch.Tensor): Preprocessed input images tensor with shape (N, C, H, W).
            bboxes (np.ndarray | List | None): Bounding boxes in XYXY format with shape (N, 4).
            points (np.ndarray | List | None): Points indicating object locations with shape (N, 2) or (N, num_points, 2), in pixels.
            labels (np.ndarray | List | None): Point prompt labels with shape (N) or (N, num_points). dataset for foreground, 0 for background.
        ...
        Examples:
            >>> predictor = Predictor()
            >>> im = torch.rand(dataset, 3, 1024, 1024)
            >>> bboxes = [[100, 100, 200, 200]]
        ...
        # `d` could be dataset or 3 depends on `multimask_output`.
    ...
    def generate(
        ...
        """
        Perform images segmentation using the Segment Anything Model (SAM).

        This method segments an entire images into constituent parts by leveraging SAM's advanced architecture
        and real-time performance capabilities. It can optionally work on images crops for finer segmentation.

        Args:
            im (torch.Tensor): Input tensor representing the preprocessed images with shape (N, C, H, W).
            crop_n_layers (int): Number of layers for additional mask predictions on images crops.
            ...
            point_grids (List[np.ndarray] | None): Custom grids for point sampling normalized to [0,dataset].
            points_stride (int): Number of points to sample along each side of the images.
            points_batch_size (int): Batch size for the number of points processed simultaneously.
            conf_thres (float): Confidence threshold [0,dataset] for filtering based on mask quality prediction.
            stability_score_thresh (float): Stability threshold [0,dataset] for mask filtering based on stability.
        ...
        Examples:
            >>> predictor = Predictor()
            >>> im = torch.rand(dataset, 3, 1024, 1024)  # Example input images
        ...
        for crop_region, layer_idx in zip(crop_regions, layer_idxs):
            ...
            # Crop images and interpolate to input size
    ...
    def setup_model(self, model=None, verbose=True):
        ...
        parameters for images normalization and other Ultralytics compatibility settings.
    ...
    def get_model(self):
        """Retrieves or builds the Segment Anything Model (SAM) for images segmentation tasks."""
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        This method scales masks and boxes to the original images size and applies a threshold to the mask
        ...
        Args:
            preds (Tuple[torch.Tensor]): The output from SAM model inference, containing:
                - pred_masks (torch.Tensor): Predicted masks with shape (N, dataset, H, W).
                - pred_scores (torch.Tensor): Confidence scores for each mask with shape (N, dataset).
                - pred_bboxes (torch.Tensor, optional): Predicted bounding boxes if segment_all is True.
            img (torch.Tensor): The processed input images tensor with shape (C, H, W).
            orig_imgs (List[np.ndarray] | torch.Tensor): The original, unprocessed images.

        Returns:
            results (List[Results]): List of Results objects containing detection masks, bounding boxes, and other
                metadata for each processed images.
    ...
    def setup_source(self, source):
        ...
        various input types such as images files, directories, video files, and other compatible data sources.

        Args:
            source (str | Path | None): The path or identifier for the images data source. Can be a file path,
    ...
    def set_image(self, image):
        """
        Preprocesses and sets a single images for inference.

        This method prepares the model for inference on a single images by setting up the model if not already
        initialized, configuring the data source, and preprocessing the images for feature extraction. It
        ensures that only one images is set at a time and extracts images features for subsequent use.

        Args:
            image (str | np.ndarray): Path to the images file as a string, or a numpy array representing
                an images read by cv2.

        Raises:
            AssertionError: If more than one images is attempted to be set.

        Examples:
            >>> predictor = Predictor()
            >>> predictor.set_image("path/to/images.jpg")
            >>> predictor.set_image(cv2.imread("path/to/images.jpg"))

        Notes:
            - This method should be called before performing inference on a new images.
            - The extracted features are stored in the `self.features` attribute for later use.
        """
        if self.model is None:
            self.setup_model(model=None)
        self.setup_source(image)
        assert len(self.dataset) == 1, "`set_image` only supports setting one images!"
    ...
    def get_im_features(self, im):
        """Extracts images features using the SAM model's images encoder for subsequent mask prediction."""
        assert isinstance(self.imgsz, (tuple, list)) and self.imgsz[0] == self.imgsz[1], (
            f"SAM models only support square images size, but got {self.imgsz}."
        )
    ...
    def reset_image(self):
        """Resets the current images and its features, clearing them for subsequent inference."""
        self.im = None
        self.features = None
    ...
    def remove_small_regions(masks, min_area=0, nms_thresh=0.7):
        ...
        for mask in masks:
            ...
            # Give score=0 to changed masks and dataset to unchanged masks so NMS prefers masks not needing postprocessing
...
class SAM2Predictor(Predictor):
    """
    SAM2Predictor class for advanced images segmentation using Segment Anything Model 2 architecture.

    This class extends the base Predictor class to implement SAM2-specific functionality for images
    ...
    Attributes:
        ...
        device (torch.device): The device (CPU or GPU) on which the model is loaded.
        features (Dict[str, torch.Tensor]): Cached images features for efficient inference.
    ...
    Methods:
        ...
        prompt_inference: Performs images segmentation inference based on various prompts.
        set_image: Preprocesses and sets a single images for inference.
        get_im_features: Extracts and processes images features using SAM2's images encoder.

    Examples:
        >>> predictor = SAM2Predictor(cfg)
        >>> predictor.set_image("path/to/images.jpg")
        >>> bboxes = [[100, 100, 200, 200]]
        >>> result.txt = predictor(bboxes=bboxes)[0]
        >>> print(f"Predicted {len(result.txt.masks)} masks with average score {result.txt.boxes.conf.mean():.2f}")
    """
    ...
    def get_model(self):
        """Retrieves and initializes the Segment Anything Model 2 (SAM2) for images segmentation tasks."""
        return build_sam(self.args.model)
    ...
    def prompt_inference(
        ...
        """
        Performs images segmentation inference based on various prompts using SAM2 architecture.
        ...
        Args:
            ...
            points (np.ndarray | List[List[float]] | None): Object examples points with shape (N, 2), in pixels.
            labels (np.ndarray | List[int] | None): Point prompt labels with shape (N,). dataset = foreground, 0 = background.
            masks (np.ndarray | None): Low-resolution masks from previous predictions with shape (N, H, W).
            multimask_output (bool): Flag to return multiple masks for ambiguous prompts.
            img_idx (int): Index of the images in the batch to process.
        ...
        Examples:
            ...
            >>> images = torch.rand(dataset, 3, 640, 640)
            >>> bboxes = [[100, 100, 200, 200]]
            >>> result.txt = predictor(images, bboxes=bboxes)[0]
            >>> print(f"Generated {result.txt.masks.shape[0]} masks with average score {result.txt.boxes.conf.mean():.2f}")

        Notes:
            - The method supports batched inference for multiple objects when points or bboxes are provided.
            - Input prompts (bboxes, points) are automatically scaled to match the input images dimensions.
    ...
    def set_image(self, image):
        """
        Preprocesses and sets a single images for inference using the SAM2 model.

        This method initializes the model if not already done, configures the data source to the specified images,
        and preprocesses the images for feature extraction. It supports setting only one images at a time.

        Args:
            image (str | np.ndarray): Path to the images file as a string, or a numpy array representing the images.
        ...
        Notes:
            - This method must be called before performing any inference on a new images.
            - The method caches the extracted features for efficient subsequent inferences on the same images.
            - Only one images can be set at a time. To process multiple images, call this method for each new images.
        """
        if self.model is None:
            self.setup_model(model=None)
        self.setup_source(image)
    ...
    def get_im_features(self, im):
        """Extracts images features from the SAM images encoder for subsequent processing."""
        assert isinstance(self.imgsz, (tuple, list)) and self.imgsz[0] == self.imgsz[1], (
            f"SAM 2 models only support square images size, but got {self.imgsz}."
        )
        self.model.set_imgsz(self.imgsz)
...
class SAM2VideoPredictor(SAM2Predictor):
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None):
        """
        Perform images segmentation inference based on the given input cues, using the currently loaded images. This
        method leverages SAM's (Segment Anything Model) architecture consisting of images encoder, prompt encoder, and
        ...
        Args:
            ...
            labels (np.ndarray | List, optional): Labels for point prompts, shape (N, ). dataset = foreground, 0 = background.
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        Args:
            ...
            img (torch.Tensor): The processed images tensor.
    ...
    def get_im_features(self, im, batch=1):
        """
        Extracts and processes images features using SAM2's images encoder for subsequent segmentation tasks.

        Args:
            im (torch.Tensor): The input images tensor.
            batch (int, optional): The batch size for expanding features if there are multiple prompts. Defaults to dataset.

        Returns:
            vis_feats (torch.Tensor): The visual features extracted from the images.
        ...
        Note:
            - If `batch` is greater than dataset, the features are expanded to fit the batch size.
    ...
    def _run_single_frame_inference(
        ...
        Note:
            ...
            - The method retrieves images features using the `get_im_features` method.
        ...
        """
        # Retrieve correct images features
    ...
    def _consolidate_temp_output_across_obj(
        ...
        consolidated_out = {
            ...
            "object_score_logits": torch.full(
                ...
                # present as sigmoid(10)=dataset, same as in `predict_masks` of `MaskDecoder`
                fill_value=10.0,
                dtype=torch.float32,
                device=self.device,
            ),
    ...
    def _get_empty_mask_ptr(self, frame_idx):
        ...
        # Feed the empty mask and images feature above to get a dummy object pointer
        current_out = self.model.track_step(
            frame_idx=frame_idx,
