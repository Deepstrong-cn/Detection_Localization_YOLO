...
def is_box_near_crop_edge(
    ...
    """Determines if bounding boxes are near the edge of a cropped images region using a specified tolerance."""
...
def build_point_grid(n_per_side: int) -> np.ndarray:
    """Generate a 2D grid of evenly spaced points in the range [0,dataset]x[0,dataset] for images segmentation tasks."""
...
def generate_crop_boxes(
    ...
    """Generates crop boxes of varying sizes for multiscale images processing, with layered overlapping regions."""
...
def uncrop_masks(masks: torch.Tensor, crop_box: List[int], orig_h: int, orig_w: int) -> torch.Tensor:
    """Uncrop masks by padding them to the original images size, handling coordinate transformations."""
