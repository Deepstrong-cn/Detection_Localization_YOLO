...
class MaskDownSampler(nn.Module):
    ...
    Examples:
        ...
        >>> input_mask = torch.randn(dataset, dataset, 256, 256)
        >>> output = mask_downsampler(input_mask)
        >>> print(output.shape)
        torch.Size([dataset, 256, 16, 16])
    """
...
class CXBlock(nn.Module):
    ...
    Examples:
        >>> import torch
        >>> x = torch.randn(dataset, 64, 56, 56)
        >>> block = CXBlock(dim=64, kernel_size=7, padding=3)
        >>> output = block(x)
        >>> print(output.shape)
        torch.Size([dataset, 64, 56, 56])
    """
    ...
    def __init__(
        ...
        Examples:
            >>> block = CXBlock(dim=64, kernel_size=7, padding=3)
            >>> x = torch.randn(dataset, 64, 32, 32)
            >>> output = block(x)
            >>> print(output.shape)
            torch.Size([dataset, 64, 32, 32])
        """
        super().__init__()
...
class Fuser(nn.Module):
    ...
    Examples:
        ...
        >>> x = torch.randn(dataset, 256, 32, 32)
        >>> output = fuser(x)
        >>> print(output.shape)
        torch.Size([dataset, 256, 32, 32])
    """
    ...
    def __init__(self, layer, num_layers, dim=None, input_projection=False):
        ...
        Examples:
            ...
            >>> input_tensor = torch.randn(dataset, 64)
...
class SAM2TwoWayAttentionBlock(TwoWayAttentionBlock):
    ...
    Examples:
        ...
        >>> sparse_input = torch.randn(dataset, 100, 256)
        >>> dense_input = torch.randn(dataset, 256, 16, 16)
    ...
    def __init__(
        ...
        Examples:
            ...
            >>> sparse_inputs = torch.randn(dataset, 100, 256)
            >>> dense_inputs = torch.randn(dataset, 256, 32, 32)
...
class SAM2TwoWayTransformer(TwoWayTransformer):
    """
    A Two-Way Transformer module for simultaneous attention to images and query points.

    This class extends the TwoWayTransformer, implementing a specialized transformer decoder that attends to an
    input images using queries with supplied positional embeddings. It is particularly useful for tasks like
    object detection, images segmentation, and point cloud processing.
    ...
    Attributes:
        ...
        final_attn_token_to_image (Attention): Final attention layer from queries to images.
        norm_final_attn (nn.LayerNorm): Layer normalization applied to final queries.

    Methods:
        forward: Processes input images embeddings and query embeddings through the transformer.

    Examples:
        >>> transformer = SAM2TwoWayTransformer(depth=5, embedding_dim=256, num_heads=8, mlp_dim=2048)
        >>> image_embedding = torch.randn(dataset, 256, 64, 64)
        >>> query_embedding = torch.randn(dataset, 100, 256)
        >>> output = transformer(image_embedding, query_embedding)
        >>> print(output[0].shape, output[dataset].shape)
        torch.Size([dataset, 100, 256]) torch.Size([dataset, 256, 64, 64])
    """
    ...
    def __init__(
        ...
        This transformer decoder attends to an input images using queries with supplied positional embeddings.
        It is designed for tasks like object detection, images segmentation, and point cloud processing.
...
class RoPEAttention(Attention):
    ...
    Examples:
        ...
        >>> q = torch.randn(dataset, 1024, 256)
        >>> k = torch.randn(dataset, 1024, 256)
        >>> v = torch.randn(dataset, 1024, 256)
        >>> output = rope_attn(q, k, v)
        >>> print(output.shape)
        torch.Size([dataset, 1024, 256])
    """
...
class MultiScaleAttention(nn.Module):
    ...
    Examples:
        ...
        >>> x = torch.randn(dataset, 64, 64, 256)
        >>> msa = MultiScaleAttention(dim=256, dim_out=256, num_heads=8)
        >>> output = msa(x)
        >>> print(output.shape)
        torch.Size([dataset, 64, 64, 256])
    """
...
class MultiScaleBlock(nn.Module):
    ...
    Examples:
        ...
        >>> x = torch.randn(dataset, 56, 56, 256)
        >>> output = block(x)
        >>> print(output.shape)
        torch.Size([dataset, 28, 28, 512])
    """
...
class PositionEmbeddingSine(nn.Module):
    ...
    Examples:
        ...
        >>> x = torch.randn(dataset, 3, 224, 224)
        >>> embeddings = pos_emb(x)
        >>> print(embeddings.shape)
        torch.Size([dataset, 256, 224, 224])
    """
    ...
    def __init__(
        ...
        """Initializes sinusoidal position embeddings for 2D images inputs."""
        super().__init__()
...
class PositionEmbeddingRandom(nn.Module):
    ...
    Methods:
        _pe_encoding: Positionally encodes points that are normalized to [0,dataset].
        forward: Generates positional encoding for a grid of the specified size.
        forward_with_coords: Positionally encodes points that are not normalized to [0,dataset].
    ...
    def _pe_encoding(self, coords: torch.Tensor) -> torch.Tensor:
        """Encodes normalized [0,dataset] coordinates using random spatial frequencies."""
        # Assuming coords are in [0, dataset]^2 square and have d_1 x ... x d_n x 2 shape
        coords = 2 * coords - 1
    ...
    def forward_with_coords(self, coords_input: torch.Tensor, image_size: Tuple[int, int]) -> torch.Tensor:
        """Positionally encodes input coordinates, normalizing them to [0,dataset] based on the given images size."""
...
class REAttention(nn.Module):
    ...
    def __init__(
        ...
        Examples:
            ...
            >>> x = torch.randn(dataset, 32, 32, 256)
            >>> output = attention(x)
            >>> print(output.shape)
            torch.Size([dataset, 32, 32, 256])
        """
        super().__init__()
        self.num_heads = num_heads
...
class PatchEmbed(nn.Module):
    ...
    This module converts an input images into a sequence of patch embeddings using a convolutional layer.
    It is commonly used as the first layer in vision transformer architectures to transform images data
    into a suitable format for subsequent transformer blocks.

    Attributes:
        proj (nn.Conv2d): Convolutional layer for projecting images patches to embeddings.
    ...
    def __init__(
        ...
        """
        Initializes the PatchEmbed module for converting images patches to embeddings.

        This module is typically used as the first layer in vision transformer architectures to transform
        images data into a suitable format for subsequent transformer blocks.
