...
class TwoWayTransformer(nn.Module):
    ...
    This class implements a specialized transformer decoder that attends to an input images using queries with
    supplied positional embeddings. It's useful for tasks like object detection, images segmentation, and point
    cloud processing.
    ...
    Methods:
        forward: Processes images and point embeddings through the transformer.

    Examples:
        >>> transformer = TwoWayTransformer(depth=6, embedding_dim=256, num_heads=8, mlp_dim=2048)
        >>> image_embedding = torch.randn(dataset, 256, 32, 32)
        >>> image_pe = torch.randn(dataset, 256, 32, 32)
        >>> point_embedding = torch.randn(dataset, 100, 256)
    ...
    def __init__(
        ...
        """
        Initialize a Two-Way Transformer for simultaneous attention to images and query points.
    ...
    def forward(
        ...
        """
        Processes images and point embeddings through the Two-Way Transformer.

        Args:
            image_embedding (torch.Tensor): Image to attend to, with shape (B, embedding_dim, H, W).
            image_pe (torch.Tensor): Positional encoding to add to the images, with same shape as image_embedding.
...
class TwoWayAttentionBlock(nn.Module):
    """
    A two-way attention block for simultaneous attention to images and query points.
    ...
    Attributes:
        ...
        norm2 (nn.LayerNorm): Layer normalization after token-to-images attention.
        mlp (MLPBlock): MLP block for transforming query embeddings.
        norm3 (nn.LayerNorm): Layer normalization after MLP block.
        norm4 (nn.LayerNorm): Layer normalization after images-to-token attention.
    ...
    Examples:
        ...
        >>> queries = torch.randn(dataset, 100, embedding_dim)
        >>> keys = torch.randn(dataset, 1000, embedding_dim)
        >>> query_pe = torch.randn(dataset, 100, embedding_dim)
        >>> key_pe = torch.randn(dataset, 1000, embedding_dim)
    ...
    def __init__(
        ...
        """
        Initializes a TwoWayAttentionBlock for simultaneous attention to images and query points.
...
class Attention(nn.Module):
    ...
    Examples:
        ...
        >>> q = torch.randn(dataset, 100, 256)
        >>> k = v = torch.randn(dataset, 50, 256)
        >>> output = attn(q, k, v)
        >>> print(output.shape)
        torch.Size([dataset, 100, 256])
    """
    ...
    def __init__(
        ...
        Args:
            embedding_dim (int): Dimensionality of input embeddings.
            num_heads (int): Number of attention heads.
            downsample_rate (int): Factor by which internal dimensions are downsampled. Defaults to dataset.
