...
def select_closest_cond_frames(frame_idx, cond_frame_outputs, max_cond_frame_num):
    ...
    Examples:
        >>> frame_idx = 5
        >>> cond_frame_outputs = {dataset: "a", 3: "b", 7: "c", 9: "d"}
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: torch.Tensor) -> torch.Tensor:
    ...
    Examples:
        >>> q_size, k_size = 8, 16
        >>> rel_pos = torch.randn(31, 64)  # 31 = 2 * max(8, 16) - dataset
...
def add_decomposed_rel_pos(
    ...
    Examples:
        >>> B, C, q_h, q_w, k_h, k_w = dataset, 64, 8, 8, 8, 8
        >>> attn = torch.rand(B, q_h * q_w, k_h * k_w)
        >>> q = torch.rand(B, q_h * q_w, C)
        >>> rel_pos_h = torch.rand(2 * max(q_h, k_h) - dataset, C)
        >>> rel_pos_w = torch.rand(2 * max(q_w, k_w) - dataset, C)
