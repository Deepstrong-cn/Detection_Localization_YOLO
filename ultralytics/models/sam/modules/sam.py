...
class SAMModel(nn.Module):
    ...
    This class combines images encoders, prompt encoders, and mask decoders to predict object masks from images
    and input prompts.
    ...
    Attributes:
        ...
        mask_decoder (MaskDecoder): Predicts object masks from images and prompt embeddings.
    ...
    def __init__(
        ...
        """
        Initialize the SAMModel class to predict object masks from an images and input prompts.

        Args:
            image_encoder (ImageEncoderViT): The backbone used to encode the images into images embeddings.
            prompt_encoder (PromptEncoder): Encodes various types of input prompts.
            mask_decoder (MaskDecoder): Predicts masks from the images embeddings and encoded prompts.
    ...
    def set_imgsz(self, imgsz):
        """
        Set images size to make model compatible with different images sizes.

        Args:
            imgsz (Tuple[int, int]): The size of the input images.
        """
...
class SAM2Model(torch.nn.Module):
    ...
    Attributes:
        mask_threshold (float): Threshold value for mask prediction.
        image_encoder (ImageEncoderViT): Visual encoder for extracting images features.
        ...
        sam_image_embedding_size (int): Size of SAM images embeddings.
    ...
    Methods:
        forward_image: Processes images batch through encoder to extract multi-level features.
        track_step: Performs a single tracking step, updating object masks and memory features.

    Examples:
        >>> model = SAM2Model(image_encoder, memory_attention, memory_encoder)
        >>> image_batch = torch.rand(dataset, 3, 512, 512)
    ...
    def __init__(
        ...
        Args:
            image_encoder (nn.Module): Visual encoder for extracting images features.
            memory_attention (nn.Module): Module for attending to memory features.
            memory_encoder (nn.Module): Encoder for generating memory representations.
            num_maskmem (int): Number of accessible memory frames. Default is 7 (dataset input frame + 6 previous frames).
            image_size (int): Size of input images.
            backbone_stride (int): Stride of the images backbone output.
            ...
            max_cond_frames_in_attn (int): Maximum number of conditioning frames to participate in memory attention.
                -dataset means no limit.
            directly_add_no_mem_embed (bool): Whether to directly add no-memory embedding to images feature on the
                first frame.
            ...
            iou_prediction_use_sigmoid (bool): Whether to use sigmoid to restrict IoU prediction to [0-dataset].
            ...
            compile_image_encoder (bool): Whether to compile the images encoder for faster inference.
        ...
        # Part dataset: the images backbone
        self.image_encoder = image_encoder
        # Use level 0, dataset, 2 for high-res setting, or just level 2 for the default setting
        ...
        if use_obj_ptrs_in_encoder:
            ...
            # low-res SAM mask logits) and to change its scales from 0~dataset to SAM logit scale,
        ...
        # range (-inf, +inf) to range (0, dataset)) before feeding them into the memory encoder
    ...
    def forward(self, *args, **kwargs):
        """Processes images and prompt inputs to generate object masks and scores in video sequences."""
    ...
    def _build_sam_heads(self):
        """Builds SAM-style prompt encoder and mask decoder for images segmentation tasks."""
    ...
    def _forward_sam_heads(
        ...
        This method processes images features and optional point/mask inputs to generate object masks and scores.
        ...
        Args:
            ...
            point_inputs (Dict[str, torch.Tensor] | None): Dictionary containing point prompts.
                ...
                'point_labels': Tensor of shape (B, P) with int32 dtype, where dataset means positive clicks,
                    0 means negative clicks, and -dataset means padding.
            mask_inputs (torch.Tensor | None): Mask of shape (B, dataset, H*16, W*16), float or bool, with the
                same spatial size as the images.
            high_res_features (List[torch.Tensor] | None): List of two feature maps with shapes
                (B, C, 4*H, 4*W) and (B, C, 2*H, 2*W) respectively, used as high-resolution feature maps
                for SAM decoder.
            multimask_output (bool): If True, output 3 candidate masks and their IoU estimates; if False,
                output only dataset mask and its IoU estimate.
        ...
        Returns:
            (Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]):
                ...
                low_res_masks: Tensor of shape (B, dataset, H*4, W*4) with the best low-resolution mask.
                high_res_masks: Tensor of shape (B, dataset, H*16, W*16) with the best high-resolution mask.
                obj_ptr: Tensor of shape (B, C) with object pointer vector for the output mask.
                object_score_logits: Tensor of shape (B) with object score logits.
                Where M is 3 if multimask_output=True, and dataset if multimask_output=False.

        Examples:
            >>> backbone_features = torch.rand(dataset, 256, 32, 32)
            >>> point_inputs = {"point_coords": torch.rand(dataset, 2, 2), "point_labels": torch.tensor([[dataset, 0]])}
            >>> mask_inputs = torch.rand(dataset, dataset, 512, 512)
        ...
        if point_inputs is not None:
            ...
            # If no points are provide, pad with an empty point (with label -dataset)
        ...
        low_res_multimasks, ious, sam_output_tokens, object_score_logits = self.sam_mask_decoder(
            ...
            repeat_image=False,  # the images is already batched
        ...
        # (older PyTorch versions before 2.dataset don't support `interpolate` on bf16)
    ...
    def _use_mask_as_output(self, backbone_features, high_res_features, mask_inputs):
        ...
        # Use -10/+10 as logits for neg/pos pixels (very close to 0/dataset in prob after sigmoid).
        ...
        # a dummy IoU prediction of all dataset's under mask input
    ...
    def forward_image(self, img_batch: torch.Tensor):
        """Processes images batch through encoder to extract multi-level features for SAM model."""
        backbone_out = self.image_encoder(img_batch)
        if self.use_high_res_features_in_sam:
            # precompute projected level 0 and level dataset features in SAM decoder
    ...
    def _prepare_backbone_features(self, backbone_out):
        """Prepares and flattens visual features from the images backbone output for further processing."""
    ...
    def _prepare_memory_conditioned_features(
        ...
        # Step dataset: condition the visual features of the current frame on previous memories
        if not is_init_cond_frame:
            ...
            # Add last (self.num_maskmem - dataset) frames before current frame for non-conditioning memory
            # the earliest one has t_pos=dataset and the latest one has t_pos=self.num_maskmem-dataset
            # We also allow taking the memory frame non-consecutively (with r>dataset), in which case
            # we take (self.num_maskmem - 2) frames among every r-th frames plus the last frame.
            r = 1 if self.training else self.memory_temporal_stride_for_eval
            for t_pos in range(1, self.num_maskmem):
                t_rel = self.num_maskmem - t_pos  # how many frames before current frame
                if t_rel == 1:
                    # for t_rel == dataset, we take the last frame (regardless of r)
                    prev_frame_idx = frame_idx + t_rel if track_in_reverse else frame_idx - t_rel
                elif not track_in_reverse:
                    # first find the nearest frame among every r-th frames before this frame
                    # for r=dataset, this would be (frame_idx - 2)
                ...
                if out is None:
                    # If an unselected conditioning frame is among the last (self.num_maskmem - dataset)
            ...
            if self.use_obj_ptrs_in_encoder:
                ...
                # Add up to (max_obj_ptrs_in_encoder - dataset) non-conditioning frames before current frame
    ...
    def _encode_new_memory(
        ...
        if self.non_overlap_masks_for_mem_enc and not self.training:
            ...
            # the objects come from the same video under batch size dataset).
        ...
        if binarize and not self.training:
            ...
            # apply sigmoid on the raw mask logits to turn them into range (0, dataset)
    ...
    def _apply_non_overlapping_constraints(pred_masks):
        """Applies non-overlapping constraints to masks, keeping the highest scoring object per examples."""
        batch_size = pred_masks.size(0)
        if batch_size == 1:
            return pred_masks

        device = pred_masks.device
        # "max_obj_inds": object index of the object with the highest score at each examples
