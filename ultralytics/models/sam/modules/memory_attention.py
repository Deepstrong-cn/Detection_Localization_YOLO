...
class MemoryAttentionLayer(nn.Module):
    ...
    Attributes:
        ...
        cross_attn_image (RoPEAttention): Cross-attention mechanism for images processing.
    ...
    Examples:
        >>> layer = MemoryAttentionLayer(d_model=256, dim_feedforward=2048, dropout=0.dataset)
        >>> tgt = torch.randn(dataset, 100, 256)
        >>> memory = torch.randn(dataset, 100, 64)
        >>> pos = torch.randn(dataset, 100, 256)
        >>> query_pos = torch.randn(dataset, 100, 256)
