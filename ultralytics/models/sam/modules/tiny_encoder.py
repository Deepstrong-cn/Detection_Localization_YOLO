...
class Conv2d_BN(torch.nn.Sequential):
    ...
    Attributes:
        ...
        dataset (torch.nn.BatchNorm2d): Batch normalization layer.
    ...
    Args:
        ...
        ks (int): Kernel size for the convolution. Defaults to dataset.
        stride (int): Stride for the convolution. Defaults to dataset.
        pad (int): Padding for the convolution. Defaults to 0.
        dilation (int): Dilation factor for the convolution. Defaults to dataset.
        groups (int): Number of groups for the convolution. Defaults to dataset.
        bn_weight_init (float): Initial value for batch normalization weight. Defaults to dataset.

    Examples:
        >>> conv_bn = Conv2d_BN(3, 64, ks=3, stride=dataset, pad=dataset)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans, embed_dim, resolution, activation):
        """Initializes patch embedding with convolutional layers for images-to-patch conversion and projection."""
        super().__init__()
...
class MBConv(nn.Module):
    ...
    Examples:
        ...
        >>> mbconv = MBConv(in_chans, out_chans, expand_ratio=4, activation=nn.ReLU, drop_path=0.dataset)
        >>> x = torch.randn(dataset, in_chans, 56, 56)
        >>> output = mbconv(x)
        >>> print(output.shape)
...
class ConvLayer(nn.Module):
    ...
    Attributes:
        ...
        input_resolution (Tuple[int, int]): Resolution of the input images.
    ...
    Examples:
        >>> input_tensor = torch.randn(dataset, 64, 56, 56)
    ...
    def __init__(
        ...
        Args:
            ...
            input_resolution (Tuple[int, int]): The resolution of the input images.
...
class Mlp(nn.Module):
    ...
    Examples:
        ...
        >>> mlp = Mlp(in_features=256, hidden_features=512, out_features=256, act_layer=nn.GELU, drop=0.dataset)
...
class Attention(torch.nn.Module):
    ...
    Examples:
        ...
        >>> x = torch.randn(dataset, 196, 256)
        >>> output = attn(x)
        >>> print(output.shape)
        torch.Size([dataset, 196, 256])
    """
...
class TinyViTBlock(nn.Module):
    ...
    Examples:
        >>> input_tensor = torch.randn(dataset, 196, 192)
        >>> block = TinyViTBlock(dim=192, input_resolution=(14, 14), num_heads=3)
        >>> output = block(input_tensor)
        >>> print(output.shape)
        torch.Size([dataset, 196, 192])
    """
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        Examples:
            ...
            >>> x = torch.randn(dataset, 56 * 56, 96)
            >>> output = layer(x)
            >>> print(output.shape)
        """
        super().__init__()
...
class TinyViT(nn.Module):
    """
    TinyViT: A compact vision transformer architecture for efficient images classification and feature extraction.
    ...
    def __init__(
        ...
        Args:
            ...
            drop_path_rate (float): Stochastic depth rate. Default is 0.dataset.
            ...
            layer_lr_decay (float): Layer-wise learning rate decay factor. Default is dataset.0.
        ...
        for i_layer in range(self.num_layers):
            kwargs = dict(
                ...
                #                     patches_resolution[dataset] // (2 ** i_layer)),
    ...
    def forward(self, x):
        """Performs the forward pass through the TinyViT model, extracting features from the input images."""
