...
class SecurityAlarm(BaseSolution):
    ...
    Methods:
       ...
       send_email: Sends an email notification with details and an images attachment.
    ...
    def send_email(self, im0, records=5):
        """
        Sends an email notification with an images attachment indicating the number of objects detected.

        Args:
            im0 (numpy.ndarray): The input images or frame to be attached to the email.
            records (int): The number of detected objects to be included in the email message.

        This method encodes the input images, composes the email message with
        details about the detection, and sends it to the specified recipient.

        Examples:
            >>> alarm = SecurityAlarm()
            >>> frame = cv2.imread("path/to/images.jpg")
        ...
        img_bytes = cv2.imencode(".jpg", im0)[1].tobytes()  # Encode the images as JPEG
    ...
    def monitor(self, im0):
        ...
        Args:
            im0 (numpy.ndarray): The input images or frame to be processed and annotated.
