...
class AIGym(BaseSolution):
    ...
    Examples:
        ...
        >>> images = cv2.imread("gym_scene.jpg")
        >>> processed_image = gym.monitor(images)
    ...
    def monitor(self, im0):
        ...
        This function processes an input images to track and analyze human poses for workout monitoring. It uses
        the YOLO Pose model to detect keypoints, estimate angles, and count repetitions based on predefined
        angle thresholds.

        Args:
            im0 (ndarray): Input images for processing.

        Returns:
            (ndarray): Processed images with annotations for workout monitoring.

        Examples:
            >>> gym = AIGym()
            >>> images = cv2.imread("workout.jpg")
            >>> processed_image = gym.monitor(images)
        """
        # Extract tracks
        ...
        self.display_output(im0)  # Display output images, if environment support display
        return im0  # return an images for writing or further usage
